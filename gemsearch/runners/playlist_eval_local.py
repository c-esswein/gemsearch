...
from gemsearch.embedding.ge_calc_word2vec import GeCalcWord2Vec
...
parser.add_argument(
    '--tags', default=True, type=bool, help='Wether to embed tags.')
parser.add_argument(
    ...
    if SHOULD_EMBED:
        if SHOULD_GENERATE_GRAPH:
            if USE_USER_IN_QUERY:
                trainingPlaylists = playlistEval.traverseAndSplitPlaylists(
            ...
            with Timer(logger=logger, message='graph generation') as t:
                ...
                idManager = IdManager(
                    outDir + 'types.csv', typeHandlers=[TypeCounter()])
                graphGenerator = GraphGenerator(outDir + 'graph.txt',
                                                idManager)

                # graphGenerator.add(data_loader.traverseTrackFeatures(dataDir+'track_features.json'))
                # add tracks without features
                for track, feature, weight in data_loader.traverseTrackFeatures(
                        dataDir + 'track_features.json'):
                    idManager.getId(track)

                graphGenerator.add(
                    data_loader.traverseTrackArtist(dataDir +
                                                    'track_artist.csv'))
                if args.tags:
                    graphGenerator.add(
                        data_loader.traverseTrackTag(dataDir +
                                                     'track_tag.csv'))
                if args.album:
                    graphGenerator.add(
                        data_loader.traverseTrackAlbum(dataDir +
                                                       'track_album.csv'))
                if args.genre:
                    graphGenerator.add(
                        data_loader.traverseArtistGenre(
                            dataDir + 'artist_genre.csv'))

                if USE_USER_IN_QUERY:
                    graphGenerator.add(
                        data_loader.traverseUserTrackInPlaylistsObj(
                            trainingPlaylists))
        ...
        if SHOULD_INDEX_ES:
            with Timer(logger=logger, message='elastic search writer') as t:
                ...
                # insert all types
                es_load_all_types(
                    data_loader.traverseTypes(outDir + 'types.csv'),
                    'music_index',
                    'music_type',
                    dismissTypes=['user'])
        ...
        name = str(config['number_walks']) + '_' + str(
            config['walk_length']) + '_' + str(config['window_size'])
        ...
        with Timer(logger=logger, message='embedding ' + name) as t:
            ...
            if config['method'] == 'deepwalk':
                ...
                em = Node2vec(
                    d=config['representation_size'],
                    max_iter=1,
                    ret_p=1,
                    inout_p=1,
                    wLen=config['walk_length'],
                    nWalks=config['number_walks'],
                    cSize=config['context_size'])
        ...
        playlistEval.loadTestLists(outDir + 'test_playlists_queries.json',
                                   True)

    # load embedding
    with Timer(logger=logger, message='ge calc initializing') as t:
        geCalc = GeCalcWord2Vec()
